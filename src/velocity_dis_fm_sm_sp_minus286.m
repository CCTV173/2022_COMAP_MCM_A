function [p_acc0,t_acc0,vtq0,t_total0]=velocity_dis_fm_sm_sp_minus286(p0,k0,t0,CP,W,N,v_in,L)
t_total=1000000;
c1=0.140; %c 1
c2=0.451; %c 2
c3=63.9; %c 3
...
for t_acc=2:dt:4
   for p_acc=p0+k0/((t_acc+5)/60+t0):dp:p0+k0/((t_acc+2)/60+t0)
       ...
       for i=1:(floor(t_acc/dt))
           vtq(i+1)=(-(c2/c3)+p_acc*55/(c3*vtq(i))-(c1*vtq(i)^2)/c3)*dt+vtq(i);
       end
       p_sin=(c1*(vtq(floor(t_acc/dt)))^2+c2)*vtq(floor(t_acc/dt));
       t_plate=(W-t_acc*(p_acc*55-CP))/(p_sin-CP);
...
vtq0(1)=v_in;
    for i=1:(floor(t_acc/dt))
        vtq0(i+1)=(-(c2/c3)+p_acc*55/(c3*vtq0(i))-(c1*vtq0(i)^2)/c3)*dt+vtq0(i);
    end
